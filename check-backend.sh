...
elif ss -tlnp 2>/dev/null | grep :5000 > /dev/null; then
    echo "✅ Port 5000 is listening"
    ss -tlnp | grep :5000
else
...
if curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:5000 | grep -q "200\|301\|302"; then
    echo "✅ Backend responds on http://127.0.0.1:5000"
    curl -I http://127.0.0.1:5000 2>&1 | head -5
else
    echo "❌ Backend does NOT respond on http://127.0.0.1:5000"
    echo "Response:"
    curl -v http://127.0.0.1:5000 2>&1 | head -10
fi
